...
To understand this concept in detail, please see [Understanding Universal Transactions](/push-chain-website/pr-preview/pr-1242/docs/chain/build/understanding-universal-transactions/).
...
Use [`encodeTxData`](/push-chain-website/pr-preview/pr-1242/docs/chain/build/utility-functions/#encode-transaction-data) to produce the correct bytes for EVM (ABI) or Solana (Anchor IDL) targets. |
...
`funds.token` takes either kind of token — see [Token Types on Push Chain](/push-chain-website/pr-preview/pr-1242/docs/chain/important-concepts/#token-types-on-push-chain) for the difference.

## Moving Prc 20 Tokens

### Moving PRC-20 Tokens (Tokens born outside Push Chain)

`funds.token` accepts a **[MoveableToken](#tx-funds)** from the supported-asset table. The origin-chain token is locked and its [PRC-20](/push-chain-website/pr-preview/pr-1242/docs/chain/important-concepts/#token-types-on-push-chain) representation is minted on Push Chain.
...
`funds.token` also accepts a **[PC-20 reference](/push-chain-website/pr-preview/pr-1242/docs/chain/important-concepts/#token-types-on-push-chain)**, a token born on Push Chain, identified by `{ chain, address }` and resolved against UniversalCore's on-chain registry instead of the SDK's static token table.
...
-   Explore end-to-end examples for each route with [Universal Transaction Scenarios](/push-chain-website/pr-preview/pr-1242/docs/chain/build/universal-transaction-scenarios/)
-   Sequence multiple chain transactions in one go with [Send Multichain Transactions](/push-chain-website/pr-preview/pr-1242/docs/chain/build/send-multichain-transactions/)
-   Trigger cross-chain execution directly from a smart contract with [Contract Initiated Multichain Execution](/push-chain-website/pr-preview/pr-1242/docs/chain/build/contract-initiated-multichain-execution/)
-   Integrate transaction flows into your frontend app using the [UI Kit](/push-chain-website/pr-preview/pr-1242/docs/chain/ui-kit/)
