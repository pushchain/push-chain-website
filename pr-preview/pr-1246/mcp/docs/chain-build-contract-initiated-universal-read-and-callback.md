...
If only your app or backend needs the value, you do not need a contract. Omit `callback` and the Universal Read Registry stores the result for you. See [Read Universal State](/push-chain-website/pr-preview/pr-1246/docs/chain/build/universal-read/).

> **Note**: The base contract's full surface, with read-only playgrounds against the deployed registry, is in [Contract Helpers](/push-chain-website/pr-preview/pr-1246/docs/chain/build/contract-helpers/#universal-read-client).
...
| Argument | Value |
| --- | --- |
| `callbackAddress` | `0x00000000000000000000000000000000000000c2`, the [Universal Read / Callback](/push-chain-website/pr-preview/pr-1246/docs/chain/setup/smart-contract-address-book/#push-chain-core-functionalities) contract. |
...
A contract that requests on its own builds the `ReadSpec` itself. The example below reads the Ethereum Sepolia ETH balance of an address and stores it by holder. It is the on-chain equivalent of the native balance read on [Read Universal State](/push-chain-website/pr-preview/pr-1246/docs/chain/build/universal-read/#evm-reads).
...
> **Note**: Solana and Web2 envelopes carry more fields. Build those with the SDK's `prepareRead` and pass `prepared.specTuple` to your entrypoint, as described in [Read Multiple Universal States](/push-chain-website/pr-preview/pr-1246/docs/chain/build/read-multiple-universal-states/).
...
`callbackDelivered` is `false` when `_onReadResult` reverted or ran out of gas. `results` is keyed by the numeric request ID, which the SDK exposes as `requestIdUint` next to the hex `requestId`. See [Track Universal Read](/push-chain-website/pr-preview/pr-1246/docs/chain/build/track-universal-read/) for resuming by request ID or transaction hash.
...
-   Learn the query grammar and read types in [Read Universal State](/push-chain-website/pr-preview/pr-1246/docs/chain/build/universal-read/)
-   Batch several reads through your receiver with [Read Multiple Universal States](/push-chain-website/pr-preview/pr-1246/docs/chain/build/read-multiple-universal-states/)
-   Resume a read from a saved request ID or transaction hash with [Track Universal Read](/push-chain-website/pr-preview/pr-1246/docs/chain/build/track-universal-read/)
-   Explore the `UniversalReadClient` base contract in [Contract Helpers](/push-chain-website/pr-preview/pr-1246/docs/chain/build/contract-helpers/#universal-read-client)
