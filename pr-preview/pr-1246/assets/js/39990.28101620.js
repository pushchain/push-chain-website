"use strict";
(self["webpackChunkpush_chain_website"] = self["webpackChunkpush_chain_website"] || []).push([[39990],{
...
;// ./node_modules/@reown/appkit-ui/node_modules/@reown/appkit-common/dist/esm/src/utils/ParseUtil.js
const ParseUtil = {
...
const OptionsController_OptionsController = {
    ...
    setEnableCoinbase(enableCoinbase) {
        OptionsController_state.enableCoinbase = enableCoinbase;
    },
    setDebug(debug) {
...
const utils_CoreHelperUtil_CoreHelperUtil = {
    ...
    createAccount(params) {
        const { chainNamespace, chainId, address } = ParseUtil_ParseUtil.parseCaipAddress(params.caipAddress);
        return {
            namespace: chainNamespace,
            address,
            chainId,
...
const controllers_OptionsController_OptionsController = {
    ...
    setEnableCoinbase(enableCoinbase) {
        controllers_OptionsController_state.enableCoinbase = enableCoinbase;
    },
    setDebug(debug) {
    ...
    onConnectMobile(wallet, wcPayUrl) {
        ...
        if (wallet?.mobile_link && wcUri) {
            try {
                ...
                const uriWithPay = utils_CoreHelperUtil_CoreHelperUtil.appendPayToUri(wcUri, wcPayUrl);
                const { redirect, redirectUniversalLink, href } = utils_CoreHelperUtil_CoreHelperUtil.formatNativeUrl(mobile_link, uriWithPay, link_mode);
...
const MobileWalletUtil = {
    ...
    isCustomDeeplinkWallet(id, namespace) {
        ...
        if (id === CUSTOM_DEEPLINK_WALLETS.PHANTOM.id) {
            return (namespace === esm_src_utils_ConstantsUtil_ConstantsUtil.CHAIN.SOLANA ||
                namespace === esm_src_utils_ConstantsUtil_ConstantsUtil.CHAIN.EVM ||
                namespace === esm_src_utils_ConstantsUtil_ConstantsUtil.CHAIN.BITCOIN);
        }
        if (id === CUSTOM_DEEPLINK_WALLETS.SOLFLARE.id) {
            return namespace === esm_src_utils_ConstantsUtil_ConstantsUtil.CHAIN.SOLANA;
        }
        if (id === CUSTOM_DEEPLINK_WALLETS.COINBASE.id) {
            return namespace === esm_src_utils_ConstantsUtil_ConstantsUtil.CHAIN.SOLANA || namespace === esm_src_utils_ConstantsUtil_ConstantsUtil.CHAIN.EVM;
        }
        if (id === CUSTOM_DEEPLINK_WALLETS.BINANCE.id) {
            return namespace === esm_src_utils_ConstantsUtil_ConstantsUtil.CHAIN.BITCOIN;
        }
        return false;
    },
    /**
    ...
    handleMobileDeeplinkRedirect(id, namespace, options) {
        ...
        if (id === CUSTOM_DEEPLINK_WALLETS.SOLFLARE.id &&
            namespace === esm_src_utils_ConstantsUtil_ConstantsUtil.CHAIN.SOLANA &&
            !('solflare' in window)) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const unsubscribeChainController = ChainController_ChainController.subscribeKey('activeCaipAddress', val => {
                if (val) {
                    ...
                    resolve(ParseUtil_ParseUtil.parseCaipAddress(val));
                }
            });
        });
    },
                    ...
                    if (event.origin === esm_src_utils_ConstantsUtil_ConstantsUtil.SECURE_SITE_SDK_ORIGIN) {
                        ...
                        try {
                            ...
                            if (authConnector && !isConnectingSocial) {
                                ...
                                resolve(ParseUtil_ParseUtil.parseCaipAddress(caipAddress));
                                EventsController_EventsController.sendEvent({
                                    type: 'track',
...
const ConnectionController_controller = {
    ...
    async connectWalletConnect({ cache = 'auto' } = {}) {
        ...
        try {
            if (cache === 'always' || (cache === 'auto' && isInTelegramOrSafariIos)) {
                ...
                wcConnectionPromise = ConnectionController_ConnectionController._getClient()?.connectWalletConnect?.();
    ...
    async switchConnection({ connection, address, namespace, closeModalOnConnect, onChange }) {
        ...
        if (caipAddress) {
            const { address: currentAddressParsed } = ParseUtil_ParseUtil.parseCaipAddress(caipAddress);
...
const ViemUtil = {
    ...
    async getViemChain(caipNetwork) {
        ...
        const { chainId } = ParseUtil_ParseUtil.parseCaipNetworkId(caipNetwork.caipNetworkId);
...
const BalanceUtil_BalanceUtil = {
    ...
    async fetchERC20Balance({ caipAddress, assetAddress, caipNetwork }) {
        ...
        const { address } = ParseUtil_ParseUtil.parseCaipAddress(caipAddress);
...
const ChainController_controller = {
    ...
    setActiveCaipNetwork(caipNetwork) {
        ...
        if (address) {
            ...
            const { address: parsedAddress } = ParseUtil_ParseUtil.parseCaipAddress(ChainController_state.activeCaipAddress);
...
const ConnectorController_controller = {
    ...
    selectWalletConnector(wallet) {
        ...
        const connector = namespace
            ...
            : undefined;
        MobileWalletUtil.handleMobileDeeplinkRedirect(connector?.explorerId || wallet.id, ChainController_ChainController.state.activeChain, { isCoinbaseDisabled: controllers_OptionsController_OptionsController.state.enableCoinbase === false });
        if (connector) {
...
function ensureCorrectAddress() {
    const { chainNamespace } = ParseUtil_ParseUtil.parseCaipNetworkId(PayController.state.paymentAsset.network);
...
async function getTransfersQuote(params) {
    const amount = NumberUtil_NumberUtil.bigNumber(params.amount)
        ...
        .toString();
    const { chainId: originChainId, chainNamespace: originChainNamespace } = ParseUtil_ParseUtil.parseCaipNetworkId(params.sourceToken.network);
    const { chainId: destinationChainId, chainNamespace: destinationChainNamespace } = ParseUtil_ParseUtil.parseCaipNetworkId(params.toToken.network);
...
function formatCaip19Asset(caipNetworkId, asset) {
    const { chainNamespace, chainId } = ParseUtil_ParseUtil.parseCaipNetworkId(caipNetworkId);
...
function isPayWithWalletSupported(networkId) {
    const { chainNamespace } = ParseUtil_ParseUtil.parseCaipNetworkId(networkId);
...
function formatBalanceToPaymentAsset(balance) {
    ...
    if (HelpersUtil_HelpersUtil.isLowerCaseMatch(balance.symbol, targetNetwork.nativeCurrency.symbol)) {
        ...
        const { address } = ParseUtil_ParseUtil.parseCaipAddress(asset);
        asset = address;
    }
    else if (!asset) {
...
const PayController = {
    ...
    async fetchTokensFromExchange() {
        ...
        const balanceWithImages = await Promise.all(allAssets.map(async (token) => {
            ...
            const { chainNamespace } = ParseUtil_ParseUtil.parseCaipNetworkId(balance.chainId);
            let address = balance.address;
            if (utils_CoreHelperUtil_CoreHelperUtil.isCaipAddress(address)) {
                const { address: parsedAddress } = ParseUtil_ParseUtil.parseCaipAddress(address);
                address = parsedAddress;
            }
    ...
    async prepareTokenLogo() {
        if (!PayController_state.paymentAsset.metadata.logoURI) {
            try {
                const { chainNamespace } = ParseUtil_ParseUtil.parseCaipNetworkId(PayController_state.paymentAsset.network);
...
let W3mPayLoadingView = class W3mPayLoadingView extends lit/* LitElement */.WF {
    ...
    renderPayment() {
        ...
        const targetNetwork = allNetworks.find(net => {
            ...
            const { chainId } = ParseUtil_ParseUtil.parseCaipNetworkId(network);
    ...
    renderWalletText() {
        ...
        const { address } = this.caipAddress ? ParseUtil_ParseUtil.parseCaipAddress(this.caipAddress) : {};
...
let W3mPayQuoteView = class W3mPayQuoteView extends lit/* LitElement */.WF {
    ...
    async fetchTokens() {
        if (this.namespace) {
            ...
            if (this.caipAddress) {
                const { chainId, chainNamespace } = ParseUtil_ParseUtil.parseCaipAddress(this.caipAddress);
    ...
    getPaymentAssetFromTokenBalances() {
        ...
        const paymentOptionsToShow = paymentOptionsWithFormattedBalances.filter(option => {
            const { chainId: optionChainId } = ParseUtil_ParseUtil.parseCaipNetworkId(option.network);
            const { chainId: paymentAssetChainId } = ParseUtil_ParseUtil.parseCaipNetworkId(this.paymentAsset.network);
    ...
    onAccountStateChanged(accountState) {
        const { address: oldAddress } = this.caipAddress
            ? ParseUtil_ParseUtil.parseCaipAddress(this.caipAddress)
            : {};
        this.caipAddress = accountState?.caipAddress;
        this.profileName = accountState?.profileName ?? null;
        if (oldAddress) {
    ...
    async onTransfer() {
        ...
        if (transferStep) {
            ...
            if (this.quote && this.selectedPaymentAsset && this.caipAddress && this.namespace) {
                const { address: fromAddress } = ParseUtil_ParseUtil.parseCaipAddress(this.caipAddress);
