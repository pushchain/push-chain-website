...
**Prerequisite:** Familiarize yourself with [Read Universal State](/push-chain-website/pr-preview/pr-1245/docs/chain/build/universal-read/) before reading this page.
...
| :: |  |  |
| `options.callback` | `{ target, gasLimit, abi, functionName, args? }` | Optional. Routes the request through your own receiver contract instead of the registry; fields in [Callback Arguments](/push-chain-website/pr-preview/pr-1245/docs/chain/build/universal-read/#callback-arguments). |
...
> `prepareRead` accepts the same subject and query options as [Read Universal State](/push-chain-website/pr-preview/pr-1245/docs/chain/build/universal-read/#read-parameters). It does not take **waitForCompletion**, **progressHook** or **advanced**; those belong to [Execute Reads](#execute-reads).
...
| `options.advanced.timeout` | `number` | Derived from expiry, at most `180000` | Maximum milliseconds this caller waits. Timing out does not cancel the request; resume it with [`trackRead`](/push-chain-website/pr-preview/pr-1245/docs/chain/build/track-universal-read/). An explicit value overrides the cap. |
...
Batch events wrap the single-read events documented in [Read Universal State](/push-chain-website/pr-preview/pr-1245/docs/chain/build/universal-read/#read-progress-hook); each read in the batch also emits its own `READ-TX-1xx` events.
...
-   **Tracking**: the progress hook streams batch events plus each read's own events. A timed-out wait cancels nothing; resume with [Track Universal Read](/push-chain-website/pr-preview/pr-1245/docs/chain/build/track-universal-read/).

## Next Steps

-   Track a batch by transaction hash with [Track Universal Read](/push-chain-website/pr-preview/pr-1245/docs/chain/build/track-universal-read/)
-   Act on results on-chain with [Contract-Initiated Universal Read and Callback](/push-chain-website/pr-preview/pr-1245/docs/chain/build/contract-initiated-universal-read-and-callback/)
-   Go back to single reads in [Read Universal State](/push-chain-website/pr-preview/pr-1245/docs/chain/build/universal-read/)
-   Find every supported chain, including `CHAIN.WEB2`, in [Constants Reference](/push-chain-website/pr-preview/pr-1245/docs/chain/build/constants/#chain-constants)
