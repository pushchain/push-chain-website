...
In the [Counter](/push-chain-website/pr-preview/pr-1245/docs/chain/tutorials/basics/tutorial-simple-counter/) and [Universal Counter](/push-chain-website/pr-preview/pr-1245/docs/chain/tutorials/basics/tutorial-universal-counter/) tutorials every increment landed on **one** Push Chain contract. Here we flip the model. A single click on a Push Chain contract increments separate counters on **Ethereum Sepolia, BNB Testnet, and Arbitrum Sepolia** at once. One contract orchestrates many.
...
This tutorial puts the [Derive Chain Executor Account (CEA)](/push-chain-website/pr-preview/pr-1245/docs/chain/tutorials/power-features/tutorial-derive-chain-executor-account/) primitive to work. If you haven't read that one yet, skim it first. We'll be deriving CEAs and authorising them on destination chains.
...
Deploy `MultiChainCounter` on Push Chain and `ExternalCounter` on each destination — Foundry, Hardhat, or Remix all work. See [smart contract deployment](/push-chain-website/pr-preview/pr-1245/docs/chain/setup/smart-contract-environment/) for setup.
...
You can also pre-authorise the CEA on third-party destination protocols this way (whitelists, vault gating, etc.) before any cross-chain activity has happened. That's the [pre-authorise pattern from the Derive CEA tutorial](/push-chain-website/pr-preview/pr-1245/docs/chain/tutorials/power-features/tutorial-derive-chain-executor-account/#example-pre-compute-and-authorize-a-push-contracts-cea).
...
Check out the next tutorial to learn how to [build a Universal Airdrop](/push-chain-website/pr-preview/pr-1245/docs/chain/tutorials/token-systems/tutorial-universal-airdrop/): one contract, every chain, no bridging.
