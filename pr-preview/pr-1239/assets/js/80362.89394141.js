...
const controller = {
    ...
    async createSwapTransaction() {
        ...
        let amount = ConnectionController/* ConnectionController */.x.parseUnits(sourceTokenAmount, sourceToken.decimals)?.toString();
        try {
            const isSourceTokenIsNetworkToken = sourceToken.address === networkAddress;
            let response = await BlockchainApiController/* BlockchainApiController */.T.generateSwapCalldata({
                userAddress: fromCaipAddress,
                from: sourceToken.address,
                to: toToken.address,
                amount: amount,
            ...
            if (isSourceTokenIsNetworkToken && amount) {
                ...
                if (nativeToken) {
                    ...
                    const rawBalance = ConnectionController/* ConnectionController */.x.parseUnits(nativeToken.quantity.numeric, decimals) ?? 0n;
                    ...
                    if (value + gasCost > rawBalance) {
                        ...
                        if (adjustedAmount > 0n) {
                            response = await BlockchainApiController/* BlockchainApiController */.T.generateSwapCalldata({
                                userAddress: fromCaipAddress,
                                from: sourceToken.address,
                                to: toToken.address,
