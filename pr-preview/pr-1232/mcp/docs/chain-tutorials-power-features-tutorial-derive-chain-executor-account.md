...
In this tutorial, you'll learn how to **derive Chain Executor Accounts (CEAs)**. A CEA is the destination-chain identity a Push Chain account uses when it executes on an external chain. This tutorial pairs with the [Derive UEA tutorial](/push-chain-website/pr-preview/pr-1232/docs/chain/tutorials/power-features/tutorial-derive-universal-executor-account/). UEAs are how external wallets execute on Push Chain; CEAs work in the opposite direction.
...
For the full SDK reference (arguments, return shape, and a live playground), see [deriveExecutorAccount in Utility Functions](/push-chain-website/pr-preview/pr-1232/docs/chain/build/utility-functions/#derive-executor-account).

#### Supported destination chains

Use `PushChain.CONSTANTS.CHAIN` values for the `chain` option. The supported set matches the chains documented in the [Smart Contract Address Book](/push-chain-website/pr-preview/pr-1232/docs/chain/setup/smart-contract-address-book/) (Ethereum Sepolia, Base Sepolia, Arbitrum Sepolia, BNB Testnet, Solana Devnet, ...).
...
The CEAFactory is deployed on each external chain. Find the per-chain address in the [Smart Contract Address Book](/push-chain-website/pr-preview/pr-1232/docs/chain/setup/smart-contract-address-book/). Unlike `IUEAFactory`, there is no precompile address for the CEAFactory; it is a regular contract.
...
Check out the next tutorial to learn how to [build universal cross-chain counters](/push-chain-website/pr-preview/pr-1232/docs/chain/tutorials/power-features/tutorial-universal-cross-chain-counters/): one Push contract, many destination chains, one transaction.
