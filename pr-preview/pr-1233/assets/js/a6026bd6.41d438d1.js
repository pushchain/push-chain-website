...
// customPropGTagEvent=send_transaction_route1_pc20_import
  ...
  async function main() {
    ...
    try {
      ...
      const outReceipt = await outTx.wait();
      const wrapperAddr = outReceipt.externalAssetAddr;
      console.log('📦 Wrapper on Sepolia:', wrapperAddr);

      // 3. Wait for the wrapper to actually land, as settlement takes a few seconds.
      const erc20 = new ethers.Contract(wrapperAddr, ['function balanceOf(address) view returns (uint256)'], provider);
      ...
      const backTx = await client.universal.sendTransaction({
        ...
        funds: {
          ...
          token: { chain: PushChain.CONSTANTS.CHAIN.ETHEREUM_SEPOLIA, address: wrapperAddr },
        },
        progressHook: (p) => console.log('[' + p.id + '] ' + p.level + ' — ' + (p.title || '')),
      });
      await backTx.wait();
...
// customPropGTagEvent=send_transaction_route2_pc20_export
  ...
  async function main() {
    ...
    try {
      ...
      const receipt = await tx.wait();
      console.log('🌐 External tx:', receipt.externalTxHash);
      console.log('📦 Wrapper on Sepolia:', receipt.externalAssetAddr);
      console.log('🔍 Explorer:', receipt.externalExplorerUrl);
    } catch (err) {
      console.error('❌ Failed:', err && err.message ? err.message : err);
    }
