...
| **Funds movement** | None. The example dispatches a payload only. The same surface supports bridging PRC-20 (`token` + `amount`); see the [Advanced Patterns](/push-chain-website/pr-preview/pr-1233/docs/chain/build/contract-initiated-examples/advanced-patterns/) for funds variants. |
