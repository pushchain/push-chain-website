"use strict";
(self["webpackChunkpush_chain_website"] = self["webpackChunkpush_chain_website"] || []).push([[523],{
...
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-nocheck
const EcosystemCard=_ref=>{let{app}=_ref;const{t}=(0,es/* useTranslation */.Bd)();const{data:twitterData}=(0,GetTwitterMetrics/* useTweetMetrics */.k)(app.twitterId||'');const[isHydrated,setIsHydrated]=(0,react.useState)(false);// Call useBaseUrl hooks unconditionally at the top
...
// @ts-nocheck
//
const EcosystemBlocks=_ref=>{let{apps}=_ref;const{t}=(0,es/* useTranslation */.Bd)();const getInitialTab=()=>{if(typeof window==='undefined')return'featured';const params=new URLSearchParams(window.location.search);if(params.get('allapps')==='true')return'all';if(params.get('partners')==='true')return'integration';if(params.get('flagship')==='true')return'featured';return'featured';};const[activeTab,setActiveTab]=(0,react.useState)(getInitialTab);const[underlineStyle,setUnderlineStyle]=(0,react.useState)({left:0,width:0});const[isLoading,setIsLoading]=(0,react.useState)(false);const tabsRef=(0,react.useRef)([]);const handleTabChange=tab=>{setIsLoading(true);setActiveTab(tab);const params=new URLSearchParams();if(tab==='featured')params.set('flagship','true');else if(tab==='integration')params.set('partners','true');else if(tab==='all')params.set('allapps','true');window.history.pushState({},'',"?"+params.toString());// Track tab change in Google Analytics
