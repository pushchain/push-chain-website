"use strict";
(globalThis["webpackChunkpush_chain_website"] = globalThis["webpackChunkpush_chain_website"] || []).push([[9100],{
...
/* unused harmony import specifier */ var defineFormatter;
...
/* unused harmony import specifier */ var block_defineFormatter;
...
const defineBlock = /*#__PURE__*/ (/* unused pure expression or super */ null && (block_defineFormatter('block', formatBlock)));
//# sourceMappingURL=block.js.map
...
/* unused harmony import specifier */ var transactionReceipt_defineFormatter;
...
const defineTransactionReceipt = /*#__PURE__*/ (/* unused pure expression or super */ null && (transactionReceipt_defineFormatter('transactionReceipt', formatTransactionReceipt)));
...
/* unused harmony import specifier */ var Bytes_Hex;
...
function Bytes_fromHex(value, options = {}) {
    ...
    if (size) {
        ...
        hex = Hex/* padRight */.M7(value, size);
    }
...
function fromNumber(value, options) {
    const hex = Bytes_Hex.fromNumber(value, options);
...
function toBigInt(bytes, options = {}) {
    ...
    if (typeof size !== 'undefined')
        assertSize(bytes, size);
    const hex = Hex/* fromBytes */.uK(bytes, options);
    return Hex/* toBigInt */.Dg(hex, options);
}
/**
...
function Bytes_toHex(value, options = {}) {
    return Bytes_Hex.fromBytes(value, options);
}
/**
...
function toNumber(bytes, options = {}) {
    ...
    if (typeof size !== 'undefined')
        assertSize(bytes, size);
    const hex = Hex/* fromBytes */.uK(bytes, options);
    return Hex/* toNumber */.Ro(hex, options);
}
/**
...
class InvalidBytesTypeError extends Errors/* BaseError */.C {
    constructor(value) {
        super(`Value \`${typeof value === 'object' ? Json/* stringify */.As(value) : value}\` of type \`${typeof value}\` is an invalid Bytes value.`, {
...
/* unused harmony import specifier */ var hmac;
/* unused harmony import specifier */ var noble_ripemd160;
/* unused harmony import specifier */ var noble_sha256;
...
/* unused harmony import specifier */ var Hash;
...
function fromAbi(abi, name, options) {
    const { args = [], prepare = true } = (options ??
        {});
    const isSelector = Hex/* validate */.tf(name, { strict: false });
    const abiItems = abi.filter((abiItem) => {
        if (isSelector) {
            if (abiItem.type === 'function' || abiItem.type === 'error')
                return getSelector(abiItem) === Hex/* slice */.di(name, 0, 4);
            if (abiItem.type === 'event')
...
function getSelector(...parameters) {
    ...
    return Hex/* slice */.di(getSignatureHash(abiItem), 0, 4);
}
// eslint-disable-next-line jsdoc/require-jsdoc
...
function getSignatureHash(...parameters) {
    ...
    if (typeof abiItem !== 'string' && 'hash' in abiItem && abiItem.hash)
        return abiItem.hash;
    return Hash_keccak256(Hex/* fromString */.sH(getSignature(abiItem)));
}
/**
...
class InvalidSelectorSizeError extends Errors/* BaseError */.C {
    constructor({ data }) {
        super(`Selector size is invalid. Expected 4 bytes. Received ${Hex/* size */.Ej(data)} bytes ("${data}").`);
        Object.defineProperty(this, "name", {
            enumerable: true,
            configurable: true,
            writable: true,
...
function decodeAddress(cursor, options = {}) {
    ...
    return [wrap(Hex/* fromBytes */.uK(Bytes_slice(value, -20))), 32];
}
/** @internal */
...
function decodeBytes(cursor, param, { staticPosition }) {
    ...
    if (!size) {
        ...
        // As we have gone wondering, restore to the original position + next slot.
        cursor.setPosition(staticPosition + 32);
        return [Hex/* fromBytes */.uK(data), 32];
    }
    const value = Hex/* fromBytes */.uK(cursor.readBytes(Number.parseInt(size, 10), 32));
    return [value, 32];
}
/** @internal */
...
function encode(preparedParameters) {
    ...
    for (let i = 0; i < preparedParameters.length; i++) {
        ...
        if (dynamic)
            ...
            staticSize += Hex/* size */.Ej(encoded);
    }
    ...
    for (let i = 0; i < preparedParameters.length; i++) {
        const { dynamic, encoded } = preparedParameters[i];
        if (dynamic) {
            staticParameters.push(Hex/* fromNumber */.oB(staticSize + dynamicSize, { size: 32 }));
            dynamicParameters.push(encoded);
            dynamicSize += Hex/* size */.Ej(encoded);
        }
        else {
    ...
    return Hex/* concat */.xW(...staticParameters, ...dynamicParameters);
}
/** @internal */
function encodeAddress(value, options) {
    ...
    return {
        dynamic: false,
        encoded: Hex/* padLeft */.Ho(value.toLowerCase()),
    };
}
/** @internal */
function encodeArray(value, options) {
    ...
    if (dynamic || dynamicChild) {
        ...
        if (dynamic) {
            const length = Hex/* fromNumber */.oB(preparedParameters.length, { size: 32 });
            return {
                dynamic: true,
                encoded: preparedParameters.length > 0 ? Hex/* concat */.xW(length, data) : length,
            };
        }
        if (dynamicChild)
    ...
    return {
        dynamic: false,
        encoded: Hex/* concat */.xW(...preparedParameters.map(({ encoded }) => encoded)),
    };
}
/** @internal */
function encodeBytes(value, { type }) {
    const [, parametersize] = type.split('bytes');
    const bytesSize = Hex/* size */.Ej(value);
    if (!parametersize) {
        let value_ = value;
        // If the size is not divisible by 32 bytes, pad the end
        // with empty bytes to the ceiling 32 bytes.
        if (bytesSize % 32 !== 0)
            value_ = Hex/* padRight */.M7(value_, Math.ceil((value.length - 2) / 2 / 32) * 32);
        return {
            dynamic: true,
            encoded: Hex/* concat */.xW(Hex/* padLeft */.Ho(Hex/* fromNumber */.oB(bytesSize, { size: 32 })), value_),
        };
    }
    ...
    return { dynamic: false, encoded: Hex/* padRight */.M7(value) };
}
/** @internal */
function encodeBoolean(value) {
    if (typeof value !== 'boolean')
        throw new Errors/* BaseError */.C(`Invalid boolean value: "${value}" (type: ${typeof value}). Expected: \`true\` or \`false\`.`);
    return { dynamic: false, encoded: Hex/* padLeft */.Ho(Hex/* fromBoolean */.xb(value)) };
}
/** @internal */
function encodeNumber(value, { signed, size }) {
    if (typeof size === 'number') {
        ...
        if (value > max || value < min)
            throw new Hex/* IntegerOutOfRangeError */.Ty({
                max: max.toString(),
                min: min.toString(),
                signed,
                size: size / 8,
    ...
    return {
        dynamic: false,
        encoded: Hex/* fromNumber */.oB(value, {
            size: 32,
            signed,
        }),
    };
...
function encodeString(value) {
    const hexValue = Hex/* fromString */.sH(value);
    const partsLength = Math.ceil(Hex/* size */.Ej(hexValue) / 32);
    const parts = [];
    for (let i = 0; i < partsLength; i++) {
        parts.push(Hex/* padRight */.M7(Hex/* slice */.di(hexValue, i * 32, (i + 1) * 32)));
    }
    return {
        dynamic: true,
        encoded: Hex/* concat */.xW(Hex/* padRight */.M7(Hex/* fromNumber */.oB(Hex/* size */.Ej(hexValue), { size: 32 })), ...parts),
    };
}
/** @internal */
function encodeTuple(value, options) {
    ...
    return {
        ...
        encoded: dynamic
            ...
            : Hex/* concat */.xW(...preparedParameters.map(({ encoded }) => encoded)),
    };
}
/** @internal */
...
/* unused harmony import specifier */ var AbiParameters_abitype;
...
function decode(parameters, data, options = {}) {
    ...
    if (Bytes_size(bytes) && Bytes_size(bytes) < 32)
        throw new DataSizeTooSmallError({
            data: typeof data === 'string' ? data : Hex/* fromBytes */.uK(data),
...
(function (encodePacked) {
    ...
    function encode(type, value, isArray = false) {
        if (type === 'address') {
            const address = value;
            Address_assert(address);
            return Hex/* padLeft */.Ho(address.toLowerCase(), isArray ? 32 : 0);
        }
        if (type === 'string')
            return Hex/* fromString */.sH(value);
        if (type === 'bytes')
            return value;
        if (type === 'bool')
            return Hex/* padLeft */.Ho(Hex/* fromBoolean */.xb(value), isArray ? 32 : 1);
        const intMatch = type.match(integerRegex);
        if (intMatch) {
            const [_type, baseType, bits = '256'] = intMatch;
            const size = Number.parseInt(bits, 10) / 8;
            return Hex/* fromNumber */.oB(value, {
        ...
        if (bytesMatch) {
            ...
            return Hex/* padRight */.M7(value, isArray ? 32 : 0);
        }
...
function AbiParameters_format(parameters) {
    return AbiParameters_abitype.formatAbiParameters(parameters);
}
/**
...
class BytesSizeMismatchError extends Errors/* BaseError */.C {
    constructor({ expectedSize, value, }) {
        super(`Size of bytes "${value}" (bytes${Hex/* size */.Ej(value)}) does not match expected size (bytes${expectedSize}).`);
        Object.defineProperty(this, "name", {
            enumerable: true,
            configurable: true,
            writable: true,
...
/* unused harmony import specifier */ var AbiConstructor_abitype;
/* unused harmony import specifier */ var AbiParameters;
...
function AbiConstructor_format(abiConstructor) {
    return AbiConstructor_abitype.formatAbiItem(abiConstructor);
}
/** @internal */
...
/* unused harmony import specifier */ var AbiItem;
...
/* unused harmony import specifier */ var SignatureErc6492_AbiParameters;
...
function SignatureErc6492_assert(wrapped) {
    if (Hex/* slice */.di(wrapped, -32) !== magicBytes)
...
function unwrap(wrapped) {
    ...
    const [to, data, signature] = SignatureErc6492_AbiParameters.decode(SignatureErc6492_AbiParameters.from('address, bytes, bytes'), wrapped);
...
function wrap(value) {
    ...
    return Hex/* concat */.xW(AbiParameters_encode(AbiParameters_from('address, bytes, bytes'), [
        to,
        data,
        signature,
    ]), magicBytes);
...
function fromBytes(signature) {
    return Signature_fromHex(Hex/* fromBytes */.uK(signature));
}
/**
...
function Signature_fromHex(signature) {
    ...
    const r = BigInt(Hex/* slice */.di(signature, 0, 32));
    const s = BigInt(Hex/* slice */.di(signature, 32, 64));
    const yParity = (() => {
...
function Signature_toHex(signature) {
    ...
    const r = signature.r;
    const s = signature.s;
    const signature_ = Signature_Hex.concat(Signature_Hex.fromNumber(r, { size: 32 }), Signature_Hex.fromNumber(s, { size: 32 }), 
    // If the signature is recovered, add the recovery byte to the signature.
    typeof signature.yParity === 'number'
        ? Signature_Hex.fromNumber(yParityToV(signature.yParity), { size: 1 })
        : '0x');
    return signature_;
}
/**
    ...
    constructor({ signature }) {
        super(`Value \`${signature}\` is an invalid signature size.`, {
            metaMessages: [
                ...
                `Received ${Hex/* size */.Ej(Hex/* from */.HT(signature))} bytes.`,
            ],
        });
        Object.defineProperty(this, "name", {
            enumerable: true,
...
class MissingPropertiesError extends Errors/* BaseError */.C {
    constructor({ signature }) {
        super(`Signature \`${Json/* stringify */.As(signature)}\` is missing either an \`r\`, \`s\`, or \`yParity\` property.`);
        Object.defineProperty(this, "name", {
            enumerable: true,
            configurable: true,
            writable: true,
...
/* unused harmony import specifier */ var Authorization_Hash;
/* unused harmony import specifier */ var Authorization_Hex;
/* unused harmony import specifier */ var Rlp;
/* unused harmony import specifier */ var Signature;
...
function hash(authorization, options = {}) {
    ...
    return Authorization_Hash.keccak256(Authorization_Hex.concat('0x05', Rlp.fromHex(Authorization_toTuple(presign
        ? {
            address: authorization.address,
...
function Authorization_toRpc(authorization) {
    ...
    return {
        address,
        chainId: Authorization_Hex.fromNumber(chainId),
        nonce: Authorization_Hex.fromNumber(nonce),
...
function Authorization_toTuple(authorization) {
    ...
    return [
        chainId ? Authorization_Hex.fromNumber(chainId) : '0x',
        address,
        nonce ? Authorization_Hex.fromNumber(nonce) : '0x',
...
/* unused harmony import specifier */ var SignatureErc8010_AbiParameters;
/* unused harmony import specifier */ var Authorization;
/* unused harmony import specifier */ var SignatureErc8010_Hex;
/* unused harmony import specifier */ var Secp256k1;
/* unused harmony import specifier */ var SignatureErc8010_Signature;
...
function SignatureErc8010_assert(value) {
    if (typeof value === 'string') {
        if (Hex/* slice */.di(value, -32) !== SignatureErc8010_magicBytes)
...
function SignatureErc8010_unwrap(wrapped) {
    ...
    const suffixLength = Hex/* toNumber */.Ro(Hex/* slice */.di(wrapped, -64, -32));
    const suffix = Hex/* slice */.di(wrapped, -suffixLength - 64, -64);
    const signature = Hex/* slice */.di(wrapped, 0, -suffixLength - 64);
...
function SignatureErc8010_wrap(value) {
    ...
    const self = Secp256k1.recoverAddress({
        ...
        signature: SignatureErc8010_Signature.from(value.authorization),
    });
    const suffix = SignatureErc8010_AbiParameters.encode(suffixParameters, [
        {
            ...value.authorization,
    ...
    const suffixLength = SignatureErc8010_Hex.fromNumber(SignatureErc8010_Hex.size(suffix), { size: 32 });
    return SignatureErc8010_Hex.concat(signature, suffix, suffixLength, SignatureErc8010_magicBytes);
}
/**
 * Validates a wrapped signature. Returns `true` if the wrapped signature is valid, `false` otherwise.
 *
...
/* unused harmony import specifier */ var bytesToHex;
...
/* unused harmony import specifier */ var ExecutionRevertedError;
/* unused harmony import specifier */ var RpcRequestError;
/* unused harmony import specifier */ var TransactionRejectedRpcError;
/* unused harmony import specifier */ var InvalidInputRpcError;
/* harmony import */ var _errors_base_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(345765);
/* harmony import */ var _errors_node_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(92592);
...
/* unused harmony import specifier */ var modular_ensureBytes;
/* unused harmony import specifier */ var modular_bytesToNumberLE;
/* unused harmony import specifier */ var modular_bytesToNumberBE;
/**
...
function hashToPrivateScalar(hash, groupOrder, isLE = false) {
    hash = modular_ensureBytes('privateHash', hash);
    const hashLen = hash.length;
    const minLen = nLength(groupOrder).nByteLength + 8;
    if (minLen < 24 || hashLen < minLen || hashLen > 1024)
        throw new Error('hashToPrivateScalar: expected ' + minLen + '-1024 bytes of input, got ' + hashLen);
    const num = isLE ? modular_bytesToNumberLE(hash) : modular_bytesToNumberBE(hash);
...
function weierstrassPoints(opts) {
    ...
    class Point {
        ...
        multiplyUnsafe(sc) {
            ...
            aInRange('scalar', sc, weierstrass_0n, N);
            const I = Point.ZERO;
...
function weierstrass(curveDef) {
    ...
    class Signature {
        constructor(r, s, recovery) {
            aInRange('r', r, weierstrass_1n, CURVE_ORDER); // r in [1..N]
            aInRange('s', s, weierstrass_1n, CURVE_ORDER); // s in [1..N]
            this.r = r;
            this.s = s;
            if (recovery != null)
    ...
    function int2octets(num) {
        aInRange('num < 2^' + nBitLength, num, weierstrass_0n, ORDER_MASK);
...
/* unused harmony import specifier */ var secp256k1_mod;
/* unused harmony import specifier */ var secp256k1_concatBytes;
/* unused harmony import specifier */ var secp256k1_numberToBytesBE;
/* unused harmony import specifier */ var secp256k1_aInRange;
/* unused harmony import specifier */ var secp256k1_bytesToNumberBE;
/* unused harmony import specifier */ var secp256k1_ensureBytes;
/* unused harmony import specifier */ var secp256k1_inRange;
...
function taggedHash(tag, ...messages) {
    ...
    if (tagP === undefined) {
        ...
        tagP = secp256k1_concatBytes(tagH, tagH);
        TAGGED_HASH_PREFIXES[tag] = tagP;
    }
    return sha256(secp256k1_concatBytes(tagP, ...messages));
}
// ECDSA compact points are 33-byte. Schnorr is 32: we strip first byte 0x02 or 0x03
const pointToBytes = (point) => point.toRawBytes(true).slice(1);
const numTo32b = (n) => secp256k1_numberToBytesBE(n, 32);
const modP = (x) => secp256k1_mod(x, secp256k1P);
const modN = (x) => secp256k1_mod(x, secp256k1N);
 ...
 */
function lift_x(x) {
    secp256k1_aInRange('x', x, secp256k1_1n, secp256k1P); // Fail if x ≥ p.
    const xx = modP(x * x);
...
const num = (/* unused pure expression or super */ null && (secp256k1_bytesToNumberBE));
/**
...
function schnorrSign(message, privateKey, auxRand = randomBytes(32)) {
    const m = secp256k1_ensureBytes('message', message);
    const { bytes: px, scalar: d } = schnorrGetExtPubKey(privateKey); // checks for isWithinCurveOrder
    const a = secp256k1_ensureBytes('auxRand', auxRand, 32); // Auxiliary random data a: a 32-byte array
...
function schnorrVerify(signature, message, publicKey) {
    const sig = secp256k1_ensureBytes('signature', signature, 64);
    const m = secp256k1_ensureBytes('message', message);
    const pub = secp256k1_ensureBytes('publicKey', publicKey, 32);
    try {
        const P = lift_x(num(pub)); // P = lift_x(int(pk)); fail if that fails
        const r = num(sig.subarray(0, 32)); // Let r = int(sig[0:32]); fail if r ≥ p.
        if (!secp256k1_inRange(r, secp256k1_1n, secp256k1P))
            return false;
        const s = num(sig.subarray(32, 64)); // Let s = int(sig[32:64]); fail if s ≥ n.
        if (!secp256k1_inRange(s, secp256k1_1n, secp256k1N))
            return false;
...
const schnorr = /* @__PURE__ */ (/* unused pure expression or super */ null && ((() => ({
    ...
    utils: {
        ...
        lift_x,
        pointToBytes,
        numberToBytesBE: secp256k1_numberToBytesBE,
        bytesToNumberBE: secp256k1_bytesToNumberBE,
        taggedHash,
        mod: secp256k1_mod,
    },
}))()));
...
/* unused harmony import specifier */ var Withdrawal;
...
class InvalidHexTypeError extends _Errors_js__WEBPACK_IMPORTED_MODULE_0__/* .BaseError */ .C {
    constructor(value) {
        super(`Value \`${typeof value === 'object' ? _Json_js__WEBPACK_IMPORTED_MODULE_2__/* .stringify */ .As(value) : value}\` of type \`${typeof value}\` is an invalid hex type.`, {
...
/* harmony export */ });
/* unused harmony exports canonicalize, parse */
