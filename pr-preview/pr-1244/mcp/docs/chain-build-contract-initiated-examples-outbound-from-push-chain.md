...
**Note**: For the conceptual background (UGPC, CEAs, fee model), see [Contract-Initiated Multichain Execution](/push-chain-website/pr-preview/pr-1244/docs/chain/build/contract-initiated-multichain-execution/).
...
| **Funds movement** | None. The example dispatches a payload only. The same surface supports bridging PRC-20 (`token` + `amount`); see the [Advanced Patterns](/push-chain-website/pr-preview/pr-1244/docs/chain/build/contract-initiated-examples/advanced-patterns/) for funds variants. |
...
The Push contract's CEA on the destination chain is computable off-chain via [deriveExecutorAccount](/push-chain-website/pr-preview/pr-1244/docs/chain/build/utility-functions/#derive-executor-account), so destination protocols can whitelist or pre-fund the CEA before the first cross-chain activity has happened.
...
| Push tx succeeds but no BNB tx fires | `gasLimit` was 0 or under the auto-floor (~500k) and the destination tx ran out of gas | Pass `gasLimit: 2_000_000` on the outbound. UGPC charges only for actual gas used and refunds the surplus, so over-provisioning is essentially free. See [Operational Knobs](/push-chain-website/pr-preview/pr-1244/docs/chain/build/contract-initiated-multichain-execution/#operational-knobs). |
...
-   Other Basic Examples → [Inbound to Push Chain](/push-chain-website/pr-preview/pr-1244/docs/chain/build/contract-initiated-examples/inbound-to-push-chain/) and the [Round-Trip with Auto Back-Leg](/push-chain-website/pr-preview/pr-1244/docs/chain/build/contract-initiated-examples/round-trip-auto-back-leg/).
-   [Advanced Patterns](/push-chain-website/pr-preview/pr-1244/docs/chain/build/contract-initiated-examples/advanced-patterns/) → Harder variants: bridging PRC-20 alongside the call, recipient bridge, FIFO state machine, three-chain cascade.
-   [How CEA Works](/push-chain-website/pr-preview/pr-1244/docs/chain/deep-dives/how-cea-works/) → The identity model behind deterministic CEA addresses on the destination chain.
