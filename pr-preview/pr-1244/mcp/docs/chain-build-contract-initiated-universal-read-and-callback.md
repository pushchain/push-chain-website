...
# Contract-Initiated Universal Read and Callback
...
If only your app or backend needs the value, you do not need a contract. Omit `callback` and the Universal Read Registry stores the result for you. See [Read Universal State](/push-chain-website/pr-preview/pr-1244/docs/chain/build/universal-read/).

> **Note**: The base contract's full surface, with read-only playgrounds against the deployed registry, is in [Contract Helpers](/push-chain-website/pr-preview/pr-1244/docs/chain/build/contract-helpers/#universal-read-client).
...
| **Refunds** | Back to the requesting account. | To `revertRecipient`. On the SDK path that is the caller's Push account (`refundTo`); a spec your contract builds with `revertRecipient` left as `address(0)` refunds to your contract. |
...
-   **Gas bound**: `callbackGasLimit` caps what your callback may consume. The hard cap is 1,000,000 gas; the SDK defaults to 500,000 (`PushChain.CONSTANTS.READ.REGISTRY_CALLBACK_GAS`) when you pass no `gasLimit`.
-   **Lifecycle**: a request moves through `PENDING`, `EXECUTED` (result delivered; the callback was attempted, even if it reverted) and `SETTLED` (gas reported, refund sent), or `EXPIRED`. `statusOf(requestId)` returns the current state; an unknown ID reads as `NONE`. Whether your callback succeeded is the `ReadFulfilled` or `CallbackFailed` event, which the SDK reports as `callbackDelivered`.
...
-   **Refunds follow `revertRecipient`**: when your contract builds the spec and leaves `revertRecipient` as `address(0)`, `_requestRead` sets it to your contract, so your contract needs a payable `receive()` or the refund is not delivered. Specs prepared by the SDK already carry the caller's Push account (or `refundTo`).
...
| Argument | Value |
| --- | --- |
| `callbackAddress` | `0x00000000000000000000000000000000000000c2`, the [Universal Read / Callback](/push-chain-website/pr-preview/pr-1244/docs/chain/setup/smart-contract-address-book/#push-chain-core-functionalities) contract. |
| `requester_` | The address that will submit requests. For an SDK caller, `pushChainClient.universal.account`. |

Be careful which address you authorize

  
`requester_` must match the **msg.sender** your receiver sees when a read is requested. For a caller using the SDK that is `pushChainClient.universal.account` on the client that will call `read`, which is not always the wallet that deployed the contract.
...
Point `read` at your receiver by passing `callback`. The SDK prepares the read, calls your payable entrypoint with the prepared `ReadSpec` and the callback gas, and waits for the result. The `callback` fields are documented in [Callback Arguments](/push-chain-website/pr-preview/pr-1244/docs/chain/build/universal-read/#callback-arguments).
...
Your entrypoint must forward the spec and gas limit it receives unchanged and call `_requestRead` exactly once itself. The SDK matches the `ReadRequested` event to the prepared read by target, gas limit and spec. If they differ, it throws `READ_REQUEST_MISMATCH` **after** the request transaction was mined and paid; resume with the hashes on the error instead of resubmitting.

### From Within Your Contract

A contract that requests on its own builds the `ReadSpec` itself. The example below reads the Ethereum Sepolia ETH balance of an address and stores it by holder. It is the on-chain equivalent of the native balance read on [Read Universal State](/push-chain-website/pr-preview/pr-1244/docs/chain/build/universal-read/#evm-reads).
...
> **Note**: Solana and Web2 envelopes carry more fields. Build those with the SDK's `prepareRead` and send them to your entrypoint with `toCallData`, which encodes `(spec, callbackGasLimit)` into your function and returns the `msg.value` to send:
> 
> ```typescript
...
> const { data, value } = toCallData(prepared, { abi: receiverAbi, functionName: 'request' }); // value === prepared.value
> const tx = await pushChainClient.universal.sendTransaction({ to: receiverAddress, data, value });
> ```
> 
> Pass `args: (spec, gasLimit) => [...]` when your entrypoint takes its arguments in another order. Prepare right before sending: the spec's expiry is fixed at prepare time.
...
`callbackDelivered` is `false` when `_onReadResult` reverted or ran out of gas, and `outcome` is then `CALLBACK_FAILED`. `results` is keyed by the numeric request ID, which the SDK exposes as `requestIdUint` next to the hex `requestId`. See [Track Universal Read](/push-chain-website/pr-preview/pr-1244/docs/chain/build/track-universal-read/) for resuming by request ID or transaction hash.
...
The SDK quotes all of this as `prepared.fees` and `prepared.value`, sizing the budget at three times `callbackGasLimit × gas price`. A contract that requests on its own must send at least `estimateFee(chainNamespace, chainId) + callbackGasLimit × block.basefee`, with headroom because the base fee can rise before fulfilment. `estimateFee` returns 0 on Donut today, so the budget is the whole requirement.

## Security Considerations

-   **Only Universal Callback can deliver**  
...
-   **The result is only as good as the request**  
...
-   **Empty bytes are an error, not a zero**  
...
-   **No reads from inside a callback**  
...
| Request reverts with `InvalidAccountId`, `EmptyQuery` or `InvalidMinConfirmations` | `account` has an empty namespace, chain ID or owner, `query` is empty, or `minConfirmations` is 0 | Fill every `account` field (any non-empty owner for EVM), build `query` as shown above, and use at least 1 confirmation. |
...
| Request reverts with `DomainBlocked` | The source chain is blocked on Universal Callback | Choose a supported source from Limitations. |
| Request succeeds but never fulfils, then `EXPIRED` | The callback budget is below `callbackGasLimit × base fee` | Send more `msg.value`; see [Fees and Refunds](#fees-and-refunds). |
...
-   Learn the query grammar and read types in [Read Universal State](/push-chain-website/pr-preview/pr-1244/docs/chain/build/universal-read/)
-   Batch several reads through your receiver with [Read Multiple Universal States](/push-chain-website/pr-preview/pr-1244/docs/chain/build/read-multiple-universal-states/)
-   Resume a read from a saved request ID or transaction hash with [Track Universal Read](/push-chain-website/pr-preview/pr-1244/docs/chain/build/track-universal-read/)
-   Explore the `UniversalReadClient` base contract in [Contract Helpers](/push-chain-website/pr-preview/pr-1244/docs/chain/build/contract-helpers/#universal-read-client)
