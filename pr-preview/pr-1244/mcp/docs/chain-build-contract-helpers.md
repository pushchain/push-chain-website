...
Inherit `UniversalReadClient` to request external state and receive it on Push Chain. `_requestRead(spec, localState, callbackGasLimit)` submits the request and preserves local context; `_onReadResult(requestId, resultData, localState)` handles delivery. The base class restricts the public callback to `UniversalCallback`.

Use a payable request entrypoint and a refund recipient that accepts native Push transfers. Authorize the Push-side account that actually submits the request: `pushChainClient.universal.account` is a UEA for an external signer and the native address for a Push signer. Do not assume the deployment EOA is the runtime caller. Requests initiated during a result callback are unsupported. See the [complete receiver example](/push-chain-website/pr-preview/pr-1244/docs/chain/build/universal-read/#integrate-your-own-contract) for constructor configuration, request ABI and SDK usage.
