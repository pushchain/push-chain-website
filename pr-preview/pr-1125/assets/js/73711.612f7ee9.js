"use strict";
(self["webpackChunkpush_chain_website"] = self["webpackChunkpush_chain_website"] || []).push([[73711],{
...
const TweetBody = ({ tweet })=>/*#__PURE__*/ (0,jsx_runtime.jsx)("p", {
...
// extracted by mini-css-extract-plugin
/* harmony default export */ const quoted_tweet_header_module = ({"header":"header_RYYj","avatar":"avatar_K4HU","avatarSquare":"avatarSquare_Zzxq","author":"author_jsWI","authorText":"authorText_dUjv","username":"username_Jiht"});
...
const config_context_client_BoS53ST9_SWRGlobalState = new WeakMap();
...
const createCacheHelper = (cache, key)=>{
    const state = config_context_client_BoS53ST9_SWRGlobalState.get(cache);
    return [
        // Getter
...
const config_context_client_BoS53ST9_serialize = (key)=>{
    if (isFunction(key)) {
        try {
            key = key();
        } catch (err) {
...
async function internalMutate(...args) {
    ...
    async function mutateByKey(_k) {
        // Serialize key
        const [key] = config_context_client_BoS53ST9_serialize(_k);
        if (!key) return;
        const [get, set] = createCacheHelper(cache, key);
        const [EVENT_REVALIDATORS, MUTATION, FETCH, PRELOAD] = config_context_client_BoS53ST9_SWRGlobalState.get(cache);
...
const initCache = (provider, options)=>{
    ...
    if (!config_context_client_BoS53ST9_SWRGlobalState.has(provider)) {
        ...
        const initProvider = ()=>{
            if (!config_context_client_BoS53ST9_SWRGlobalState.has(provider)) {
                ...
                config_context_client_BoS53ST9_SWRGlobalState.set(provider, [
                    EVENT_REVALIDATORS,
                    Object.create(null),
                    Object.create(null),
                    Object.create(null),
                ...
                if (!IS_SERVER) {
                    ...
                    unmount = ()=>{
                        ...
                        config_context_client_BoS53ST9_SWRGlobalState.delete(provider);
                    };
                }
            }
        };
    ...
    return [
        provider,
        config_context_client_BoS53ST9_SWRGlobalState.get(provider)[4]
    ];
};
...
const [config_context_client_BoS53ST9_cache, mutate] = initCache(new Map());
// Default config
const defaultConfig = mergeObjects({
    ...
    compare,
    isPaused: ()=>false,
    cache: config_context_client_BoS53ST9_cache,
    mutate,
    fallback: {}
...
const SWRConfig = (props)=>{
    ...
    if (provider && !cacheContextRef.current) {
        cacheContextRef.current = initCache(provider(extendedConfig.cache || config_context_client_BoS53ST9_cache), config);
    }
...
const middleware = (useSWRNext)=>(key_, fetcher_, config)=>{
        ...
        const fetcher = fetcher_ && ((...args)=>{
            const [key] = config_context_client_BoS53ST9_serialize(key_);
            const [, , , PRELOAD] = config_context_client_BoS53ST9_SWRGlobalState.get(config_context_client_BoS53ST9_cache);
...
const useSWRHandler = (_key, fetcher, config)=>{
    ...
    const [key, fnArg] = config_context_client_BoS53ST9_serialize(_key);
