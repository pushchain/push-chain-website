...
- EIP-7702 atomic batch execution for native Push Chain EOAs: multicall batches now execute in a single type-4 transaction via the `PushBatchExecutor` when the signer can sign authorizations (ethers v6 `Wallet`, viem local accounts) — all calls succeed or the whole transaction reverts.
- Added `atomic: boolean` to the universal transaction response. Wallets that cannot sign an EIP-7702 authorization (e.g. browser JSON-RPC wallets like MetaMask) log a warning and safely fall back to the legacy sequential per-call execution (`atomic: false`).
- Archive RPC fallback on Push Chain Donut Testnet: history-sensitive reads (transactions, receipts, universal-tx status) automatically retry the archive endpoints when the default pruned RPC has no record.

#### Fixes

- Gas-swap preflight no longer caps the buffered WPC swap estimate below the swap's real minimum, so tight balances surface a real shortfall instead of an on-chain Uniswap `STF` revert.
- Failed Push-side transactions now surface normalized, human-readable revert messages instead of raw logs.
- Multicall entries with a zero `to` address are now rejected (the EIP-7702 executor reinterprets a zero target as the account itself).

#### Other Changes

- Outbound wait-stage progress markers now include the coordinating Push Chain tx hash (`pushTxHash`) in event metadata.
...
- refresh internal viem/wagmi types and Buffer polyfill handling; no public component or prop changes.
