...
The SDK defaults to this registry only on `TESTNET_DONUT`. It calls `read(spec, queryKey, callbackGasLimit)` with 500,000 callback gas by default. Other networks need a verified deployment or an application receiver. See [Universal Read](/docs/chain/build/universal-read/).
