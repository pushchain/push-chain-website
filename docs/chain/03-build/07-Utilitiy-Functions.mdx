...
Converts a human-readable string representation of a number to a scaled integer (bigint) by multiplying by 10^exponent. This is commonly used for handling token amounts where you need to convert from human-readable decimals to the smallest unit (like converting ETH to wei).

```typescript
const result = PushChain.utils.helpers.parseUnits('1.5', 18);
// Returns: 1500000000000000000n (1.5 ETH in wei)
```
...
| _`exponent`_  | `number` | The number of decimal places to scale by. Must be a non-negative integer. For example, use `18` for ETH, `6` for USDC, `8` for BTC.              |
