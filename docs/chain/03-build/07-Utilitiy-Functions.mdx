...
**_`PushChain.utils.account.convertExecutorToOrigin(ueaAddress): Promise<{ account: UniversalAccount | null; isUEA: boolean }>`_**

Gives the origin account (UOA) for a given executor account (UEA).

```typescript
const { account, isUEA } = await PushChain.utils.account.convertExecutorToOrigin(ueaAddress);
```
...
<Details summary="Returns `{ account: UniversalAccount | null; isUEA: boolean }` <object>" className="alert alert--fn-args">

```typescript
// { account: UniversalAccount | null; isUEA: boolean }
{
  account: {
    chain: 'eip155:11155111',
    address: '0xFd6C2fE69bE13d8bE379CCB6c9306e74193EC1A9'
  },
  isUEA: true
}
```
...
import { PushChain } from '@pushchain/core'
    ...
    async function main() {
      const ueaAddress = '0xc16a585b95810F7D204620bb3677F73243242A8F';
