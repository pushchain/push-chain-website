...
{`
    ...
    // ——— CONFIG ———
    // change RPC to sepolia or solana to target sending transaction from them
    ...
    async function main() {
      ...
      // 6) Readline to ask for input
      const rl = readline.createInterface({
        input: process.stdin,
        output: process.stdout
      });
      const answer = await rl.question(':::prompt:::Please send funds to: ' + wallet.address + ' to continue.');
      rl.close();
    
      // 7) Send universal transaction
      ...
      try {
        ...
        const txReceipt = await pushChainClient.universal.sendTransaction(txParams);
        console.log('✅ Transaction sent! Hash:', JSON.stringify(txReceipt));
      } catch (error) {
