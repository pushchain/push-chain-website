...
| `tx.progressHook`         | `(progress: ProgressHookType) => void` | A callback function to receive progress updates during transaction lifecycle, especially for cross-chain transactions. |

> Note: These _`Arguments`_ are mandatory.

<Details summary="ProgressHook Type and Response" className="alert alert--minimal">
