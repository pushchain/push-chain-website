...
You can pay the origin-chain gas and Push Chain settlement fees using supported ERC-20 tokens (e.g., USDT) instead of the native coin. Use the `funds.payWith` field to specify the token to pay fees with.

```typescript
// Pay gas with USDT while bridging 1 USDT and calling a contract on Push Chain
const COUNTER_ABI = [
...
Notes:
- Ensure the account holds sufficient balance of the selected `payWith.token` on the origin chain to cover gas and settlement fees.
