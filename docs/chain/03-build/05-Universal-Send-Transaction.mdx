...
Below are end-to-end examples showing how to use the new `funds` parameter.
...
<Tabs className="liveplaytab" groupId="send-universal-funds-examples">
  ...
  ### Example A: Funds + payload (ERC-20 only, origin = Sepolia)

  When you pass a `funds` object, the SDK performs a “move + execute” flow in a single call: it first locks the specified asset on the origin chain (here, Sepolia), then credits your Universal Execution Address (UEA) on Push Chain with the Push-native representation, and finally (if `data` is provided) executes the call on the target `to` contract on Push Chain. Use `client.moveable.token.<SYMBOL>` to choose supported ERC-20 tokens for the origin, and fund the generated address with both ETH (for gas) and the ERC-20 you plan to move.
  ...
  - Replace `funds: { amount }` with `funds: { amount, token: client.moveable.token.USDT }` to bridge ERC-20 explicitly.
  - Native token with payload is not supported yet.

  </TabItem>

  <TabItem value='funds_erc20_only' label='ERC-20 Funds Only'>

  ### Example B: Bridge ERC-20 funds only (no payload)

  This moves an ERC-20 from the origin to your UEA on Push Chain without executing calldata. Fund the generated address with Sepolia ETH (for gas) and the ERC-20 you want to move.
  ...
  ### Example C: Bridge native funds only (no payload)

  This bridges the native asset from the origin to your UEA without calldata. Fund the generated address with Sepolia ETH to cover gas.
...
- `MoveableToken` includes: `symbol`, `decimals`, `address`, and `mechanism`.
- Access tokens via `client.moveable.token.<SYMBOL>`; common options are `ETH`, `USDT`, `WETH` (availability depends on chain).
