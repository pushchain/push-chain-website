...
**Behind the scenes**, the SDK automatically **estimates gas, assembles cross-chain signature and replay any EVM or non-EVM proofs**, so you can focus on your app instead of what, why and where of different chains.
...
**_`pushChainClient.universal.sendTransaction({ .. }): Promise<TransactionReceipt>`_**
...
| `progressHook`                 | `(progress: ProgressHookType) => void` | A callback function to receive progress updates during transaction lifecycle, especially for cross-chain transactions.             |
...
| `progress.id`       | `string` | Predefined ID for the progress.       |
| `progress.infoType` | `string` | Predefined InfoType for the progress. |
| `progress.Type`     | `string` | The type of the transaction.          |
| `progress.Data`     | `object` | The data of the transaction.          |
...
import * as readline from 'node:readline/promises';
    ...
    function returnPushChainTestnetConfig(chainMeta) {
      // define Viem config
      return defineChain({
        id: chainMeta.id === '1' ? 42101 : 11155111, // Push Testnet or Sepolia
        name: chainMeta.name,
        nativeCurrency: {
          decimals: 18,
          name: chainMeta.id === '1' ? 'PC' : 'ETH',
          symbol: chainMeta.id === '1' ? '$PC' : 'ETH',
        },
        rpcUrls: {
          default: {
            http: [chainMeta.id === '1' ? RPC_PUSH : RPC_SEPOLIA],
          },
        },
        blockExplorers: {
          default: {
            name: chainMeta.id === '1' ? 'Push Testnet Explorer' : 'Sepolia Explorer',
            url: chainMeta.id === '1' ? 'https://donut.push.network/' : 'https://sepolia.etherscan.io/',
          },
        },
      });
    }
...
<Details summary="Live Playground: Progress Lifecycle of Transaction" className="alert alert--live-play">
