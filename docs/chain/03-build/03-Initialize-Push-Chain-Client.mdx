...
- **moveable.token**: `MoveableTokenAccessor` for tokens that can be bridged/moved on Push Chain
...
Use `funds.getConversionQuote(amountIn, { from, to }) → Promise<ConversionQuote>` to fetch a spot quote for converting a payable token into a moveable token on supported origins.

<Details summary="Getting `ConversionQuote` <object> from `funds.getConversionQuote`">
<br />
Calling `funds.getConversionQuote(amountIn, { from, to })` returns a `Promise<ConversionQuote>` with the computed spot quote.
<br />

```typescript
const amountIn = PushChain.utils.helpers.parseUnits('0.005', 18);
const quote = await pushChainClient.funds.getConversionQuote(amountIn, {
  from: pushChainClient.payable.token.WETH,
  to: pushChainClient.moveable.token.USDT,
});
```
