...
**_`pushChainClient.universal.executeTransactions(txs: PreparedUniversalTx[], options?: { progressHook?: (event: ProgressEvent) => void }): Promise<CascadedTxResponse>`_**
...
Pass `options.progressHook` to receive the unified SDK `ProgressEvent` stream for this cascade, including pre-flight checks, broadcast, and cascade tracking events. This is additive with the client-level `progressHook` passed to `PushChain.initialize`.
...
| `options.progressHook` | `(event: ProgressEvent) => void` | Optional per-call callback for unified SDK progress events. Receives the same event shape as `sendTransaction` progress hooks. |
...
| `eventHook` | `(event: ProgressEvent) => void` | `options.progressHook` from `executeTransactions` | Unified SDK progress callback during cascade tracking. |
...
- **Tracking**: Pass `{ progressHook }` as the second `executeTransactions` argument for the unified SDK `ProgressEvent` stream. Use `cascade.wait({ progressHook })` when you need the lower-level per-hop `CascadeProgressEvent` shape.
