...
Use a payable request entrypoint and a refund recipient that accepts native Push transfers. Authorize the Push-side account that actually submits the request: `pushChainClient.universal.account` is a UEA for an external signer and the native address for a Push signer. Do not assume the deployment EOA is the runtime caller. Requests initiated during a result callback are unsupported. See the [complete receiver example](/docs/chain/build/universal-read/#integrate-your-own-contract) for constructor configuration, request ABI and SDK usage.
