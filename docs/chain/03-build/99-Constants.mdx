...
| `PushChain.CONSTANTS.CHAIN.WEB2` | `web2:https` | Web2 / HTTPS endpoint. A read-only destination for [Read Universal State](/docs/chain/build/universal-read/), not a transaction destination |
...
Values used by [Read Universal State](/docs/chain/build/universal-read/). Use them to compare a read's status and outcome, and to find the contracts that receive reads on Push Chain.
