...
Batch events wrap the single-read events documented in [Read Universal State](/docs/chain/build/universal-read/#read-progress-hook); each read in the batch also emits its own `READ-TX-1xx` events. The `READ-TX-001`, `002` and `999` events below fire only when more than one read is executed.
