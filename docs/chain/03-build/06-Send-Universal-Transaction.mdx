...
| `tx.funds`        | `{ amount: BigInt; token?: MoveableToken \| PC20TokenReference }` | Moves supported assets as part of the transaction. If `tx.data` is provided, the movement and the call happen atomically.<br /><br />Pass a `MoveableToken`, or `{ chain, address }` for a [PC-20](#moving-pc-20-tokens). <ConstantsDropdown variant="MOVEABLE_TOKEN" /> |
...
<h2 className="anchor" id="moving-prc-20-tokens" style={{ height: '0px', overflow: 'hidden', margin: 0, padding: 0 }}>Moving Prc 20 Tokens</h2>
...
<h2 className="anchor" id="moving-pc-20-tokens" style={{ height: '0px', overflow: 'hidden', margin: 0, padding: 0 }}>Moving Pc 20 Tokens</h2>
...
:::danger Never add `symbol` to a PC-20 reference
