...
<h2 className="anchor" id="tx-funds" style={{ height: '0px', overflow: 'hidden', margin: 0, padding: 0 }}>Transaction Parameters</h2>
...
| `tx.funds`        | `{ amount: BigInt; token?: MoveableToken \| PC20TokenReference }` | Moves supported assets as part of the transaction. If `tx.data` is provided, the movement and the call happen atomically.<br /><br />Pass a `MoveableToken`, or `{ chain, address }` for a [PC20](#moving-pc20-tokens). <ConstantsDropdown variant="MOVEABLE_TOKEN" /> |
...
`funds.token` takes either kind of token — see [Token Types on Push Chain](/docs/chain/important-concepts#token-types-on-push-chain) for the difference.

### Moving PRC-20 Tokens (Tokens born outside Push Chain)

`funds.token` accepts a **[MoveableToken](#tx-funds)** from the supported-asset table. The origin-chain token is locked and its [PRC-20](/docs/chain/important-concepts#token-types-on-push-chain) representation is minted on Push Chain.
...
`funds.token` also accepts a **[PC-20 reference](/docs/chain/important-concepts#token-types-on-push-chain)**, a token born on Push Chain, identified by `{ chain, address }` and resolved against UniversalCore's on-chain registry instead of the SDK's static token table.
...
**Important:** `funds.chain` is the chain the token sit on right now. It is different from the destination `to.chain` which indicates where it should go.

:::danger Never add `symbol` to a PC20 reference

A PC20 reference is exactly `{ chain, address }`. The SDK tells it apart from a `MoveableToken` by the **absence** of `symbol` and `mechanism`, adding a `symbol` field silently routes the token down the static-table path instead.
