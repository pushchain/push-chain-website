...
const source = readPages
  .map(page => fs.readFileSync(new URL('../docs/chain/03-build/' + page, import.meta.url), 'utf8'))
  .join('\n');
...
for (const sample of samples) {
  ...
  assert.match(sample.code, sample.id === 'universal_read_prepare' ? /finally\s*\{[\s\S]*provider\.destroy\(\)/ : /finally\s*\{[\s\S]*rl\.close\(\)/);
...
assert.equal(inboxFences.length, 1, 'Expected exactly one ExternalStateInbox solidity fence');
...
assert.equal((readContracts.UniversalCallback.proxy ?? readContracts.UniversalCallback.address).toLowerCase(), PushChain.CONSTANTS.READ.UNIVERSAL_CALLBACK_ADDRESSES.TESTNET_DONUT.toLowerCase());
...
const addressBook = fs.readFileSync(new URL('../docs/chain/02-setup/02-Smart-Contract-Address-Book.mdx', import.meta.url), 'utf8');
const addressBookProxy = name => {
  const row = addressBook.split('\n').find(line => line.startsWith('| ' + name + ' '));
...
assert.equal(readContracts.UniversalReadRegistry.proxy.toLowerCase(), registryAddress.toLowerCase());
assert.equal(constants.READ.UNIVERSAL_READ_REGISTRY_ADDRESS.TESTNET_DONUT.toLowerCase(), registryAddress.toLowerCase());
assert.equal(readContracts.UniversalCallback.proxy.toLowerCase(), addressBookProxy('Universal Read / Callback').toLowerCase());
  ...
  try {
    for (const sample of samples) {
      ...
      const log = { log: (...args) => {
        ...
        if (args[0] === (sample.id === 'universal_read_prepare' ? 'Total fee:' : 'Value:')) printedValue = true;
